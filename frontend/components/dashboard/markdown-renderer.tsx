...
const markdownComponents: Components = {
  ...
  img({ alt, src }) {
    ...
    return (
      <figure className="my-8 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.035]">
        <div className="flex aspect-[16/9] items-center justify-center bg-[radial-gradient(circle_at_20%_20%,rgba(45,212,191,0.18),transparent_34%),linear-gradient(135deg,rgba(255,255,255,0.08),rgba(255,255,255,0.02))] p-6 text-center text-sm text-zinc-400">
          {source ? (
            <span className="font-mono text-xs text-zinc-500">{source}</span>
          ) : null}
        </div>
        {alt ? (
  ...
  p({ children }) {
    return <p className="my-4 text-[15px] leading-7 text-zinc-300">{children}</p>
  },
  pre({ children }) {
    return <>{children}</>
  },
