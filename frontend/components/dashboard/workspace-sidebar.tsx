...
export function WorkspaceSidebar({
  ...
  return (
    <aside
      className={cn(
        "fixed left-0 top-0 z-40 flex h-screen flex-col border-r border-white/10 bg-black/30 backdrop-blur-2xl transition-all duration-300",
          ...
          >
            {isLoadingBlogs ? (
              <div
                className={cn(
                  "flex flex-col",
                  collapsed ? "items-center gap-3" : "gap-3",
                )}
              >
                {[1, 2, 3].map((item) => (
                  <div
                ...
                >
                  <div
                    className={cn(
                      ...
                      activeSessionId === session.id && "text-teal-200",
                    )}
                  >
